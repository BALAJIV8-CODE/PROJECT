...
def encode_image():
    ...
    encode_wn.config(bg='White')
    Label(encode_wn, text='Encode an Image', font=("Comic Sans MS", 15), bg='White').place(x=220, rely=0)

    Label(encode_wn, text='Enter the path to the image(with extension):', font=("Times New Roman", 13),
          bg='White').place(x=10, y=50)
    Label(encode_wn, text='Enter the data to be encoded:', font=("Times New Roman", 13), bg='White').place(
        x=10, y=90)
    ...
    Button(encode_wn, text='Encode the Image', font=('Helvetica', 12), bg='black', fg = 'white' ,command=lambda:
...
def decode_image():
    ...
    decode_wn.config(bg='White')

    Label(decode_wn, text='Decode an Image', font=("Comic Sans MS", 15), bg='White').place(x=220, rely=0)
    ...
    Button(decode_wn, text='Decode the Image', font=('Helvetica', 12), bg='black', fg =  'white' ,command=lambda:
    main_decryption(img_entry.get(), text_strvar)).place(x=220, y=90)

    Label(decode_wn, text='Text that has been encoded in the image:', font=("Times New Roman", 12), bg='White').place(
        x=180, y=130)
...
Label(root, text='Image Steganography', font=('Comic Sans MS', 15), bg='White',
      wraplength=300).place(x=40, y=0)

Button(root, text='Encode', width=25, font=('Times New Roman', 13), bg='black', fg = 'white', command=encode_image).place(
    x=30, y=80)

Button(root, text='Decode', width=25, font=('Times New Roman', 13), bg='black', fg = 'white', command=decode_image).place(
    x=30, y=130)
